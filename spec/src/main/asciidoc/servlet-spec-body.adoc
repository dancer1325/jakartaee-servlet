...
* compatible implementation (CI) (TODO: Which one?) -- has been made -- available /
    ** behavioral benchmark for this specification
    ** uses
        *** comprehend better a specification / leaves implementation of a particular feature open to interpretation

* Jakarta EE Platform TCK
    ** goal
        *** assessing whether implementations -- meet the -- compatibility requirements of the Jakarta Servlet API standard
...
* Web server and application server vendors / want to provide servlet engines -- conform to -- this standard

* Authoring tool developers / want to support web applications -- conform to -- this specification.

* Experienced servlet authors / want to understand the underlying mechanisms of servlet technology.

=== API Reference

* Check
    ** `https://jakarta.ee/specifications/servlet` or
    ** "../api"
...
* http://www.ietf.org/rfc/[RFCs], related to the
    ** development and implementation of the
        *** Jakarta Servlet API
        *** Jakarta standard servlet engines
    ** Check
...
* World Wide Web Consortium (`http://www.w3.org/`)
    ** source of HTTP related information
* XML (eXtensible Markup Language)
    ** uses
        *** specification of the Deployment Descriptors -- Chapter 13 --
...
* := Jakarta technology-based web component /
    ** -- managed by a -- container
        *** -> run | web server
    ** generates dynamic content
    ** -- interacts, via request-response paradigm ( / -- implemented by -- servlet engines),with -- web clients
* == platform-independent Java classes /
    ** -- compiled to -- platform-neutral byte code /
        *** -- can be loaded dynamically into -- Jakarta technology-enabled web server
        *** -- run by -- Jakarta technology-enabled web server
* servlet engines or containers
    ** == web server extensions / provide servlet functionality

=== What is a Servlet Container?

* == part of a web server OR application server /
    ** provides
        *** the network services over which
    ...
    ** how to build?
        *** -- built into a -- host web server or
        *** as add-on component | web server -- via -- server’s native extension API
    ** support
        *** (mandatory) HTTP and HTTPS as protocols -- for -- requests and responses
            **** required versions
                ***** HTTP/1.1
                ***** HTTP/2
                    ****** -> servlet containers -- must support -- “h2” and “h2c” protocol identifiers -> -- must support -- ALPN
        *** (optional) additional request/response-based protocols
    ** -- may place -- security restrictions | environment in which a servlet executes
* Java SE 17+ | Java platform / servlet containers must be built

=== An Example

Typical sequence of events:

* client (e.g., a web browser) -- accesses a -- web server
* client -- makes an -- HTTP request /
    ** -- is received by -- the web server / -- handed off to -- servlet container /
        *** servlet container can be running in
            **** process == as the host web server process
            **** process != host web server process, BUT same host
            **** host != host web server
* servlet container
    ** -- determines, based on the configuration of its servlets, -- which servlet / invoke
    ** -- calls -- this specific servlet / pass
        *** object / -- represent the -- request
        *** object / -- represent the -- response
* servlet
    ** -- based on the request object, determine ---
        *** who the remote user is
        *** what HTTP `POST` parameters / -- may have been sent as -- part of this request
        *** other relevant data
    ** performs whatever logic / was programmed with,
    ** generates data / -- sent back, via the response object, to -- the client
* servlet container
    ** if servlet has finished processing the request
        *** -- ensures that -> the response is properly flushed
        *** -- returns control back to -- host web server
...
* vs Common Gateway Interface (CGI)
    ...
    ** much faster
        *** Reason: 🧠 different process model is used 🧠
* vs other web frameworks (_Example:_ Jakarta Server Faces)
    ** lower level abstraction
* standard API / -- supported by -- many web servers
* ALL advantages of the Java programming language
    ** ease of development
    ** platform independence
* -- access to -- large set of APIs / available for the Java platform

== The Servlet Interface

* == ⭐ central abstraction of the Jakarta Servlet API ⭐
    ** implemented by ALL servlets
        *** directly OR
        *** indirectly (== extend a class / implements the interface)
    ** classes | Jakarta Servlet API / implement the `Servlet` interface
        *** `GenericServlet`
        *** `HttpServlet`
            **** 👁️most common one 👁️
...
* `service()`
    ** allows
        *** handling client requests /
            **** to handle concurrent requests -> design servlets / can deal with multiple threads executing | `service` / at a particular time
    ** called / request / servlet container -- routes to an -- instance of a servlet

==== HTTP Specific Request Handling Methods

* `HttpServlet` is abstract subclass /
    ** adds methods beyond the basic `Servlet` interface / called by the `service()` -- to aid in -- processing HTTP-based requests
...
* by default, -- directly calls -- `doGet` method
* implement HEAD behaviour of the HTTP protocol -- based on -- the container

==== Additional Methods

* `doPut()` and `doDelete()` methods
    ** -> Servlet Developers -- support -- HTTP/1.1 clients (-> these features)
* `doOptions()`
    ** responds with which HTTP methods / supported by the servlet
* `doTrace()`
    ** generates a response / contain ALL instances of the headers -- sent in the -- `TRACE` request
* `CONNECT` HTTP method
    ** https://www.ietf.org/rfc/rfc2817.txt[RFC2817]
    ** rejected by Servlet Containers /
        *** return `SC_NOT_IMPLEMENTED` (501) response
        *** -- NOT pass the request to -- a Filter or Servlet
        *** Reason: 🧠 vs Jakarta Servlet API 🧠
            **** `CONNECT` -- applies to -- proxies
            **** Jakarta Servlet API -- targets at -- endpoints
        *** if you want to handle `CONNECT` HTTP requests -> Servlet Containers should provide container specific functionality

==== Sensitive headers

* Check `HttpServlet.isSensitiveHeader`
* recommended by RFC 9110

==== Conditional GET Support

* conditional `GET` operation -- 👁️ ONLY if it has been modified since a specified time 👁️ -- sends the resource
    ** -- done via -- `getLastModified`

=== Number of Instances

* ways to specify
    ** annotation
        *** Check <<Annotations and Pluggability>>
    ** deployment descriptor of the web application / contains the servlet
        *** Check <<Deployment Descriptor>>
* if servlet NOT hosted | distributed environment -> servlet container -- must use -- ONLY 1 instance / servlet declaration
* if servlet -- was deployed as part of an -- application / marked in the deployment descriptor as distributable ->  >= 1 instance / servlet declaration / Java Virtual Machine (JVM™)

=== Servlet Life Cycle

* -- managed by -- Servlet Container
* steps
    ** load and instantiate -- == | API -- `init`
    ** initialize -- == | API -- `init`
...
* can occur when
    ** container is started OR
    ** container -- determines -- servlet is needed to service a request
* how does it work?
    ** servlet container -- locates the -- servlet classes
    ** servlet container -- via normal Java class loading,  loads the -- servlet class
        *** available loading, from
            **** local file system
            **** remote file
            **** other network services
    ** servlet container -- instantiates -- it

==== Initialization

* 👁️ instantiate != initialize 👁️
* -- required previously to -- handle requests from clients
    ** Example:
        *** read persistent configuration data
        *** initialize costly resources (such as JDBC™ API-based connections)
        *** perform other one-time activities
* -- done via -- `init(ServletConfig)`
* Check <<Servlet Context>>

===== Error Conditions on Initialization

* `UnavailableException`
    ** if it indicates minimum time of unavailability -> container -- must wait before -- instantiate a new one
        *** else, immediately after the failure
* `ServletException`
    ** servlet
        *** must NOT be placed | active service
        *** must be released -- by the -- servlet container
    ** `destroy` method is NOT called
        *** Reason: 🧠unsuccessful initialization 🧠
    ** immediately after this failure -> a new instance, by the container, -- may be -- instantiated

===== Tool Considerations

* static initialization methods invocation | tool loads and introspects a web application (?) != `init`  invocation
* once `Servlet.init` is called -> servlet is | active container runtime
    ** -> -- can try to establish -- connections (_Example:_ to DDBB, Jakarta Enterprise Beans, ...)
    ** 👁️NOT before 👁️

==== Request Handling

* requirements
    ** servlet is properly initialized
* requests
    ** -- are represented by -- `ServletRequest`
        *** pass it as -- Servlet's `service(ServletRequest)`
        *** if it's concretely an HTTP request -- represented by -- `HttpServletRequest`
* servlet's response
    ** -- are represented by -- `ServletResponse`
        *** pass it as -- Servlet's `service(ServletResponse)`
        *** if it's concretely an HTTP request -- represented by -- `HttpServletResponse`
* servlet instance -- may handle -- NO requests | its lifetime
    ** Possible reasons
        *** NO traffic
        *** Incorrect Mapping
        *** Application Design
...
* use case
    ** servlet container -- sends, through the Servlet's `service` --  concurrent requests
* recommendations
    ** make adequate provisions -- for -- concurrent processing
    ** NOT synchronize the `service` method (or methods dispatched to it)
        *** Reason: 🧠 detrimental effects | performance 🧠
...
* `UnavailableException`
    ** == servlet -- is unable to handle requests -- either temporarily or permanently
        *** if it's permanent ->
            **** container must
                ***** remove the servlet -- via -- `destroy`
                ***** release a new servlet instance
            **** `SC_NOT_FOUND` (404) response got / any request
        *** if it's temporary -> container -- could route to -- another servlet
            **** `SC_SERVICE_UNAVAILABLE` (503) & `Retry-After` header / any request
* `ServletException`
...
* resources | .jar's `META-INF/resources`
    ** if `getRealPath()` is done & container has unpacked them from their containing
JAR file ->
        *** consider it
...
* Check https://docs.oracle.com/javase/tutorial/essential/concurrency/[Java Tutorial on Multithreaded Programming]
...
* 👁️ if a web application does NOT contain ANY servlet, filter, or listener components OR annotations / -- used to -- declare them -> NOT required to contain a web.xml 👁️
   ** == application / contain ONLY static files or JSP pages
...
* goal
   ** Jakarta Servlet Specification requirements -- for -- web container support of deployment descriptors
* Deployment descriptor
   ** -- conveys -- the web application's elements & configuration information, between
      *** Application Developers,
      *** Application Assemblers,
      *** Deployers
   ** -- is defined in terms of -- XML schema document
      *** link:https://jakarta.ee/xml/ns/jakartaee/[actual .xsd]
   ** backwards compatibility
      *** == applications / written to previous versions of the Servlet API, are still supported

=== Deployment Descriptor Elements

* types of configuration + deployment information / are supported | web application deployment descriptor / ALL servlet containers
...
* rules about the processing of the deployment descriptor | web application
   ** Web containers -- must remove -- ALL leading and trailing whitespace / -- defined as -- `S(white space)` in link:http://www.w3.org/TR/2000/WD-xml-2e-20000814[XML 1.0]
| deployment descriptor's text nodes' element content

   ** deployment descriptor -- must be valid against -- the schema
      *** Web containers and tools / manipulate web applications,
         **** -- have a -- wide range of options / check the validity of
            ***** a WAR
            ***** WAR's deployment descriptor document
         **** -- provide a -- level of semantic checking
            ***** _Example:_ check that a role / referenced | security, 's name == one of the deployment descriptor's security roles
         **** if non-conformant web applications -> tools and containers -- should inform the developer with -- descriptive error messages
            ***** recommendations: application server vendors -- should supply this validity checking as -- tool / -- separate from the -- container
   ** sub elements | `web-app` -- can be in an -- arbitrary order
      *** multiplicity of the elements -- was changed from -- `optional` - to -> [0,ANY)
         **** referred elements are
            ***** `distributable`
            ***** `session-config`
            ***** `welcome-file-list`
            ***** `jsp-config`
            ***** `login-config`
            ***** `locale-encoding-mapping-list` 
         **** Reason: 🧠 restriction of XML Schema 🧠
         **** if deployment descriptor contains >1 element `session-config`, `jsp-config`, and `login-config` -> containers -- should inform the developer with -- descriptive error messages
         **** if there are multiple occurrences -> container -- must concatenate the items in -- `welcome-file-list` and `locale-encoding-mapping-list`
         **** treatment of multiple occurrence of `distributable` == treatment of 1! occurrence of `distributable`
   ** URI paths / specified | deployment descriptor -- are assumed to be in -- URL-decoded form
      *** if URL contains `CR(#xD)` or `LF(#xA)`  -> containers -- should inform the developer with -- descriptive error messages
      *** containers -- must preserve -- ALL other characters (including whitespace | URL)
   ** Containers -- must attempt to canonicalize -- paths | deployment descriptor
      *** _Example:_ paths of the form `/a/../b` -- must be interpreted as -- `/b`
         **** Reason: 🧠 Paths beginning or resolving to paths that begin with `../` -- are NOT valid paths | deployment descriptor 🧠
   ** URI paths / refer to a resource relative to the root of the WAR, OR a path mapping relative to the root of the WAR -- unless otherwise specified, should begin with a -- leading `"/"`
   ** if elements / value is an enumerated type -> value is case sensitive

=== Deployment Descriptor

* deployment descriptor specification `https://jakarta.ee/xml/ns/jakartaee/web-app_{spec-version-underscore}.xsd`

=== Examples

* how to use the deployment descriptor schema
